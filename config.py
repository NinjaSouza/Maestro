...
class TNLoopConfig:
    ...
    ENABLE_TN_COUPLING:       bool  = True
    RELAXATION_FACTOR:        float = 0.5
    MAX_TN_ITERATIONS:        int   = 20
    CONVERGENCE_EPSILON_TEMP: float = 0.5    # ΔT máximo [K] — reduzido de 1.0→0.5
    CONVERGENCE_EPSILON_POWER: float = 0.01  # ΔP relativo máximo (1%)
    CONVERGENCE_EPSILON_RHO:  float = 0.001  # Δρ máximo (0.1%)
    MAX_TEMPERATURE_K:        float = 3500.0
    MIN_TEMPERATURE_K:        float = 273.0
    DYNAMIC_RELAXATION:       bool  = False
    PICARD_MAX_INTERNAL:      int   = 10     # ↑ de 5→10: mais iterações internas
